...
gboolean
shoes_gtk_app_cmdline (GApplication *application, gchar ***arguments, gint *exit_status)
{
...
// Find a Canvas with an active scrollbar [or a touch method attatched]
// Climbs up tree. Returns first one found or NULL if none.
// 
static shoes_canvas *shoes_gtk_touch_find(shoes_canvas *canvas) {
    // we might have a canvas in found. Do we have an active scrollbar?
    if (canvas != NULL && canvas->slot->vscroll != NULL) {
      //fprintf(stderr, "Search: have canvas + scrollbar\n");
      return canvas;
    }
    if (canvas != NULL && !NIL_P(canvas->parent)) {
      //fprintf(stderr, "Search climb\n");
      shoes_canvas *cvs;
      TypedData_Get_Struct(canvas->parent, shoes_canvas, &shoes_canvas_type, cvs);
      return shoes_gtk_touch_find(cvs);  //climb up
    }
    //fprintf(stderr, "Search: nothing found\n");
    return NULL;
}

// data arg is a VALUE - an App or a Canvas
static gboolean shoes_canvas_gtk_touch(GtkWidget *widget, GdkEventTouch *event, shoes_canvas *data) {
    shoes_canvas *canvas;
    if (data == NULL || (VALUE)data == Qnil) {
      //fprintf(stderr," Touch: NIL for app/canavas\n");
      return FALSE;
    }
    canvas = shoes_gtk_touch_find(data);
    if (canvas == NULL) {
      //fprintf(stderr, "Touch: nothing found\n");
      return FALSE; 
    }
    GtkWidget *sb = canvas->slot->vscroll;
    if (sb == NULL) {
    ...
    if (event->type == GDK_TOUCH_BEGIN) {
      ...
      canvas->app->touch_x = event->x;
      canvas->app->touch_y = event->y;
    } else if (event->type == GDK_TOUCH_END) {
      canvas->app->touch_x = 0.0;
      canvas->app->touch_y = 0.0;
      ...
      gdouble dy = (gdouble)canvas->app->touch_y - event->y;
    ...
    return FALSE; // false => We did not handle the event - need this for 
                  // click emulation to happen. TODO
}

static gboolean shoes_gtk_app_touch(GtkWidget *widget, GdkEventTouch *event, gpointer data) {
  //fprintf(stderr,"App Touch\n");
  shoes_app *app = (shoes_app *)data;
  shoes_canvas *canvas;
  if (data == NULL)
    return FALSE;
  TypedData_Get_Struct(app->canvas, shoes_canvas, &shoes_canvas_type, canvas);
  return shoes_canvas_gtk_touch(widget, event, canvas);
}
static gboolean shoes_gtk_slot_touch(GtkWidget *widget, GdkEventTouch *event, gpointer data) {
  //fprintf(stderr,"Slot Touch\n");
  return shoes_canvas_gtk_touch(widget, event, data);
}
...
shoes_code shoes_native_app_open(shoes_app *app, char *path, int dialog, shoes_settings *st) {
   ...
   char icon_path[SHOES_BUFSIZE];
    ...
    // rpi touch screen
    geo.height -= 32 ;
    if (app->width > geo.width) 
      app->width = geo.width;
    if (app->height > geo.height)
      app->height = geo.height;
    ...
    g_signal_connect(G_OBJECT(window), "touch-event",
                     G_CALLBACK(shoes_gtk_app_touch), (gpointer)app);
...
// TODO: Appears to not be used at Shoes/ruby level
...
void shoes_native_slot_init(VALUE c, SHOES_SLOT_OS *parent, int x, int y, int width, int height, int scrolls, int toplevel) {
    ...
    */
    ...
    g_signal_connect(G_OBJECT(slot->oscanvas), "touch-event",
                     G_CALLBACK(shoes_gtk_slot_touch), (gpointer)c);
#endif
...
static void shoes_gtk_root_size(GtkWidget *widget, cairo_t *cr, gpointer data) {
    ...
    if (wdg != NULL ) {
       gtk_widget_set_size_request(GTK_WIDGET(wdg), -1, app->height);
    }
...
static void shoes_gtk_content_size(GtkWidget *widget, GtkAllocation *size, gpointer data) {
    ...
    if (canvas->slot->vscroll) {
        ...
        if (size->height != canvas->slot->scrollh) {
          ...
          fprintf(stderr, "alloc: %d %d %d %d\n\n", alloc.x, alloc.y, alloc.width, alloc.height);
#endif
          gtk_fixed_move(GTK_FIXED(canvas->slot->oscanvas), GTK_WIDGET(canvas->slot->vscroll), size->width - alloc.width, 0);
          gtk_adjustment_set_page_size(adj, size->height);
          gtk_adjustment_set_page_increment(adj, size->height - 32);
...
gboolean shoes_app_gtk_configure_menu(GtkWidget *widget, GdkEvent *evt, gpointer data) {
  shoes_app *app = (shoes_app *)data;
  if (widget == app->os.window) {  // GtkWindow
    gtk_window_resize(GTK_WINDOW(widget), evt->configure.width, evt->configure.height);
    shoes_canvas *canvas;
    TypedData_Get_Struct(app->canvas, shoes_canvas, &shoes_canvas_type, canvas);
#if GTK_CHECK_VERSION(3,12,0)
    ...
    if (shoes_gtk_is_maximized(app, evt->configure.width, evt->configure.height)) {
      ...
      fprintf(stderr, "cfg alloc: %d %d %d %d\n\n", alloc.x, alloc.y, alloc.width, alloc.height);
#endif
      gtk_fixed_move(GTK_FIXED(canvas->slot->oscanvas), GTK_WIDGET(canvas->slot->vscroll),
				     evt->configure.width - alloc.width, 0);
      gtk_adjustment_set_page_size(adj, evt->configure.height);
      gtk_adjustment_set_page_increment(adj, evt->configure.height - 32);
...
void shoes_slot_init_menu(VALUE c, SHOES_SLOT_OS *parent, int x, int y, int width, int height, int scrolls, int toplevel) {
  ...
  */
  ...
  if (toplevel) {
    ...
    gtk_fixed_put(GTK_FIXED(parent->oscanvas), GTK_WIDGET(slot->oscanvas), x, y);
  }
  slot->scrollh = slot->scrollw = 0;
  slot->vscroll = NULL;
  if (scrolls) {
    ...
    gtk_fixed_put(GTK_FIXED(slot->oscanvas), GTK_WIDGET(slot->vscroll), -100, -100);
    gtk_widget_set_size_request(GTK_WIDGET(slot->oscanvas), width, height);
  ...
  if (toplevel) {
    ...
    gtk_widget_show_all(GTK_WIDGET(slot->oscanvas));
...
shoes_code shoes_native_app_open_menu(shoes_app *app, char *path, int dialog, shoes_settings *st) {
    ...
    gtk_box_pack_start(GTK_BOX(vbox), GTK_WIDGET(shoes_window), FALSE, FALSE, 0);
   ...
   // commit https://github.com/shoes/shoes/commit/4e7982ddcc8713298b6959804dab8d20111c0038
    ...
    g_signal_connect(GTK_WIDGET(window), "touch-event",
                    G_CALLBACK(shoes_gtk_app_touch), app);
