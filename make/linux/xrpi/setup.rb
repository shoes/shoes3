...
module Make
  ...
  # so_list arg unused. uses Constants instead. 
  def static_setup (so_list)
    ...
    # copy include files so users can build gems
    ...
    if APP['THEME']
       puts "Copy #{APP['THEME']} Theme"
       mkdir_p "#{TGT_DIR}/themes"
       cp_r "themes/#{APP['THEME']}", "#{TGT_DIR}/themes"
       File.open("#{TGT_DIR}/startup.yaml", 'w') do |f|
         f.write "Theme: #{APP['THEME']}"
       end
    end
  end
end
