Shoes.app(title: "Drag the dot to make a path. Redrag to extend.") do
   @path = [[width / 2, height / 2, RADIUS = 20]]

   @controller = nil
   click do |_btn, left, top|
      px, py, pr = @path.last
      if left.between?(px - pr, px + pr) && top.between?(py - pr, py + pr)
         @controller = true
      end
   end
   release do |_btn, _left, _top|
      @controller = nil
   end
