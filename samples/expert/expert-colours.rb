...
def generate_rgb(rgb_start, rgb_end, segments = 2)
   rstep, gstep, bstep = 3.times.collect do |n|
...
Shoes.app(width: (SQUARE_SIZE + SQUARE_PADDING) * SQUARES + SQUARE_PADDING, height: (SQUARE_SIZE + SQUARE_PADDING) * HUES.size + SQUARE_PADDING, title: TITLE % 0, resizable: false) do
   flow(top: SQUARE_PADDING, left: SQUARE_PADDING / 2, width: (SQUARE_SIZE + SQUARE_PADDING) * SQUARES)  do
      ...
      @hues.collect { |n| [n.first, n[1..-2].shuffle, n.last].flatten }.each_with_index do |list, index|
         SQUARES.times do |n|
   ...
   @anim = animate(10) do |frame|
      if 0 == frame % 5
         ...
         if @highlight
            @hues.flatten.each_with_index do |item, index|
   ...
   start do
      @highlight = false
      @r.each_with_index do |n, skip|
         next if (0 == skip % SQUARES) || ((SQUARES - 1) == (skip % SQUARES))
         n.click do |_btn, _left, _top|
            if @success
               set_window_title(TITLE % (@completed = 0))
               @hues.collect { |n| [n.first, n[1..-2].shuffle, n.last].flatten }.flatten.each_with_index do |item, index|
            ...
            if @controller.nil?
               @controller = n
               @top_rect.left = n.left
               @top_rect.top = n.top
               @top_rect.fill = n.fill
               @top_rect.stroke = n.stroke
               @top_rect.show
            else
