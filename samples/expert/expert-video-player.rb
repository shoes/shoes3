...
class Shoes::Knob < Shoes::Widget
   ...
   def tweak(origin_left, args = nil)
      ...
      app.motion do |lf, _t|
         ...
         if @active && (!@released) && @range.cover?(pos)
            self.fraction = pos
            if args
      ...
      app.release {|_b, _l, _t| @released = true}
   end
end
   ...
   stack do
      ...
      @controls = flow margin: [0, 10, 0, 0], height: 47 do
         ...
         @vol_knob = knob driven, fraction: start_vol, padding: 20, size: 0.75, color: red,
                         click: proc { |_but, left, _t| @vol_knob.tweak(left) }
      end
      ...
      @url_slot = flow hidden: true, margin_bottom: 5 do
         ...
         button "Play" do
            unless @url_edit.text.nil? || @url_edit.text.empty?
               @anim.stop
