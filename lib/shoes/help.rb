...
module Shoes::Manual
   ...
   def self.path
      ...
      path = "#{DIR}/static/manual-en.txt" unless File.exists? path
      path
   end
   ...
   def dewikify_hi(str, terms, intro = false)
      if terms
         ...
         str = str
           ...
           .gsub(/CODE#\[(\d+)\]/) { code[Regexp.last_match(1).to_i] }
      end
      dewikify(str, intro)
   end
   ...
   def wiki_tokens(str, intro = false)
      ...
      paras.map do |ps|
         if ps =~ CODE_RE
            yield :code, Regexp.last_match(1)
         else
            case ps
            when /\A\{COLORS\}/
               yield :colors, nil
            ...
            when /\A\{SAMPLES\}/
               yield :samples, nil
            when /\A \* (.+)/m
               yield :list, Regexp.last_match(1).split(/^ \* /)
            when /\A==== (.+) ====/
               yield :caption, Regexp.last_match(1)
            when /\A=== (.+) ===/
               yield :tagline, Regexp.last_match(1)
            when /\A== (.+) ==/
               yield :subtitle, Regexp.last_match(1)
            when /\A= (.+) =/
               yield :title, Regexp.last_match(1)
            else
               yield :para, ps
            end
         end
 ...
 #    eval(str, TOPLEVEL_BINDING)
        ...
        (str.split(/^= (.+?) =/)[1..-1] / 2).map do |k, v|
           ...
           sections = (sparts[1..-1] / 2).map do |k2, v2|
              ...
              hsh = {"title" => k2, "section" => k,
                "description" => meth[0],
                "methods" => (meth[1..-1] / 2).map do |_k, _v|
   ...
   def open_link(head)
      if head == "Search"
         show_search
      elsif @sections.key? head
         open_section(head)
      elsif @methods.key? head
         open_methods(head)
      elsif @mindex.key? head
   ...
   def open_methods(meth_s, terms = nil, meth_a = nil)
      ...
      @doc.clear do
         ...
         meth_h["methods"].each do |mname, expl|
            if meth_a.nil? || (meth_a == mname)
               ...
               stack(margin_top: 8, margin_bottom: 8) do
                 background "#333".."#666", curve: 3, angle: 90
                 tagline sig, (span("»", val, stroke: "#BBB") if val), margin: 4 end
      ...
      optn = nil
      docn = begin
                @docs.index { |_, h| optn = h["sections"].index { |x,| x == meth_s } }
             rescue
                nil
             end
   ...
   def manual_search(terms)
      ...
      @search.find_all(terms).map do |title, _count|
         title.split(" ", 2)
      end
   end
            ...
            docs.each do |sect_s, sect_h|
               ...
               @toc[sect_cls] =
                 stack hidden: @toc.empty? ? false : true do
                    links = sect_h["sections"].map do |meth_s, _meth_h|
