module PackShoes
   ...
   require "fileutils"
# ----- create a shy ------
   def self.make_shy(dest_path, in_dir, desc)
...
# ----- Windows -----
   ...
   def self.dnlif_exe(opts)
     ...
     # puts "win script = #{script}"
      ...
      exe.inject_icons(opts["ico"]) if opts["ico"]
      exe.save(script.gsub(/\.\w+$/, "") + ".exe")
      f.close
   end

   def self.repack_exe(opts, &blk)
      script = custom_installer opts
      size = File.size(script)
      f = File.open(script, "rb")
      ...
      blk.call "Repack Shoes.exe #{opts['shoesdist']} distribution" if blk
      exe.inject_file(Winject::EXE::SHOES_SYS_SETUP, f2.read)
      exe.save(script.gsub(/\.\w+$/, "") + ".exe") do |len|
      end
      f.close
      f2.close
      blk.call "Done packaging Windows" if blk
   end

# ------ Linux ------

   def self.dnlif_linux(opts)
      arch = opts["arch"]
      script = custom_installer opts
      name = File.basename(script).gsub(/\.\w+$/, "")
      app_name = name.capitalize.gsub(/[-_](\w)/) { Regexp.last_match(1).capitalize }
     ...
     # compress tar file
      ...
      md5 = Shy.md5sum(tgz_path)
      fsize = File.size(tgz_path)
      ...
      FileUtils.chmod 0o755, run_path
      FileUtils.rm_rf(tgz_path)
      FileUtils.rm_rf(tmp_dir)

   end

   def self.repack_linux(opts, &blk)
     # opts.each {|k, v| puts "#{k} => #{v}"}
      ...
      blk.call"Expanding #{arch} distribution. Patience is needed" if blk
    ...
    # Copy the rest to a new file
      ...
      buff = ""
      wkf.write(buff) while pkgf.read(32_768, buff) != nil
      wkf.close
      blk.call "Start extract" if blk
      @tarmodes = {}
      fastxzf(wkf, tmp_dir, @tarmodes)
      FileUtils.rm_rf(wk_dir)
      blk.call "Copy script and stubs" if blk
      FileUtils.cp(script, File.join(tmp_dir, File.basename(script)))
      File.open(File.join(tmp_dir, "sh-install"), "wb") do |a|
         rewrite a, File.join(DIR, "static", "stubs", "sh-install"),
                 "SCRIPT" => "./#{File.basename(script)}"
      end
      FileUtils.chmod 0o755, File.join(tmp_dir, "sh-install")
     # add sh-install and script to the modes list
      @tarmodes[File.basename(script)] = "0755".oct
      @tarmodes["sh-install"] = "0755".oct
     # debug - dump @tarmodes
     # @tarmodes.each { |k,v| puts "#{k} #{sprintf('%4o',v)}" }
      ...
      blk.call "Compute size and compress" if blk
      raw = Shy.du(tmp_dir)
     ...
     # compress tar file
      ...
      FileUtils.rm tar_path
      blk.call "Checksum, install  and copy" if blk
      md5 = Shy.md5sum(tgz_path)
      fsize = File.size(tgz_path)
      File.open(run_path, "wb") do |f|
         rewrite f, File.join(DIR, "static", "stubs", "blank.run"),
      ...
      blk.call "Done packing Linux" if blk
      FileUtils.chmod 0o755, run_path
      FileUtils.rm_rf(tgz_path)
      FileUtils.rm_rf(tmp_dir)
   end
...
# ------ OSX -----
   ...
   def self.dnlif_osx(opts)
     ...
     # @tarmodes["#{app_app}/Shoes.icns"] = 0644
      @tarmodes["#{app_app}/Contents/Resources/#{icon_name}"] = 0o644
      @tarmodes["#{app_app}/#{icon_name}"] = 0o644
     # more permissions cache entries
      ["#{app_app}", "#{app_app}/Contents", "#{app_app}/Contents/Resources",
       "#{app_app}/Contents/MacOS"].each {|d| @tarmodes[d] = 0o755}
      ["#{app_app}/Contents/PkgInfo", "#{app_app}/Contents/version.plist",
       "#{app_app}/Contents/Info.plist"].each {|f| @tarmodes[f] = 0o644}
...
END
      ...
      FileUtils.chmod 0o755, ls
      @tarmodes["#{app_app}/Contents/MacOS/#{name}-launch"] = 0o755
      FileUtils.cp(script, File.join(mac_dir, File.basename(script)))
      @tarmodes["#{app_app}/Contents/MacOS/#{File.basename(script)}"] = 0o644
     # copy the downloader
      FileUtils.cp(File.join(DIR, "static", "stubs", "shoes-osx-install"),
                   File.join(mac_dir, "shoes-osx-install"))
      @tarmodes["#{app_app}/Contents/MacOS/shoes-osx-install"] = 0o755
     ...
     # make the 1st script to run (installs shoes if needed, runs Shoes
      ...
      @tarmodes["#{app_app}/Contents/MacOS/osx-app"] = 0o755
  ...
  #    FileUtils.rm_rf(tmp_dir)
   ...
   def self.repack_osx(opts, &blk)
     ...
     # expand download into ~/.shoes/+dmg/Shoes.app
      ...
      blk.call"Expanding OSX distribution. Patience is needed" if blk
...
END
     ...
     #  File.join(mac_dir, "cocoa-install"))
      blk.call "Creating new archive" if blk
  ...
  # only sub matches.
   def self.rewrite(a, before, hsh)
      File.open(before) do |b|
         b.each do |line|
            a << line.gsub(/\#\{(\w+)\}/) do
               if hsh[Regexp.last_match(1)]
                  hsh[Regexp.last_match(1)]
               else
                  '#{' + Regexp.last_match(1) + "}"
               end
            end
   ...
   def self.fastxzf(infile, outdir, modes = {}, osx = "")
   ...
   def self.fastcf(outf, indir, modes)
      begin
   ...
   def self.custom_installer(opts)
      defshy = opts["app"]
      return defshy if opts["advopts"] != true
     # opts.each {|k, v| puts "#{k} => #{v}"}
      raise "Must be a shy" unless defshy[/\.shy$/]
     ...
     # return defshy
      File.join(shydir, "#{appname}-custom.shy")
   end

   def self.tar_extract(opened_file)
