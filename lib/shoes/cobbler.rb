...
class Gem::CobblerFace
   class DownloadReporter  # ProgressReporter
      ...
      def updated(_message)
         @count += 1.0
         @prog.fraction = (@count / @total.to_f) * 0.5
      end

      def fetch(_filename, len)
         @total = len
      end
  ...
  # init CobblerFace
   ...
   def alert(_msg, quiz = nil)
      ask(quiz) if quiz
   end

   def download_reporter(*_args)
      ...
      File.open(after, "w") do |a|
         File.open(before) do |b|
            b.each do |line|
               a << line.gsub(reg) do
                  if reg2.include? '\1'
                     reg2.gsub(/\\1/, ENV[Regexp.last_match(1)])
                  else
                     reg2
                  end
               end
      ...
      but it needs to know where Shoes is"
                ...
                flow do
                   ...
                   button "Select..." do
                      ...
                      @shoes_loc.text = this_shoes if this_shoes
                   end
                end
                new_loc = "#{ENV['HOME']}"
                flow do
                   ...
                   button "Select ..." do
                      ...
                      @cshoes_loc.text = new_loc if new_loc
                   end
                end
                flow do
                   para "Ready? "
                   button "Create Script" do
                      if confirm "Create #{@cshoes_loc.text}/cshoes Using #{@shoes_loc.text}"
                        # prepare for rewrite - expect complaints about constants
                         ...
                         chmod 0o755, "#{@cshoes_loc.text}/cshoes"
                      end
                   end
                end
             end
