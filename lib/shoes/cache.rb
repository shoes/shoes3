...
if tight_shoes
   ...
   $LOAD_PATH.unshift SITE_LIB_DIR
   $LOAD_PATH.unshift GEM_DIR
   ENV["GEM_HOME"] = GEM_DIR
else
  # puts "LOOSE Shoes #{RUBY_VERSION} #{DIR}"
   $LOAD_PATH.unshift ENV["GEM_HOME"] if ENV["GEM_HOME"]
   ...
   $LOAD_PATH.unshift DIR + "/lib/ruby/#{rv}/#{RbConfig::CONFIG['arch']}"
   $LOAD_PATH.unshift DIR + "/lib/ruby/#{rv}"
   $LOAD_PATH.unshift DIR + "/lib/shoes"
  ...
  # or file contents
   if File.exist? jloc
      open(jloc, "r") do |f|
         f.each do |l|
            ln = l.strip
            $LOAD_PATH.unshift ln if ln.length > 0
         end
      end
      ENV["GEM_PATH"].split(":").each {|p| $LOAD_PATH.unshift p } if ENV["GEM_PATH"]
      require "rubygems"
