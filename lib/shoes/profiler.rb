  ...
  # $stderr.puts "got it: #{$shoes_profiler.file}"
   ...
   def index
      ...
      stack do
       ...
       para "You don't have a cpu_clock, using wall time instead" unless $cpu_clock
       flow do
   ...
   def filter_by(filter)
      max = $shoes_profiler.nodes.sort_by { |n, _mi| n.length }[-1][0].length
      sorted = $shoes_profiler.nodes.sort_by { |_n, mi| mi.send(filter) }
      ...
      stack do
         ...
         flow  do
            button "Counts", margin: 4 do
               ...
               @result_slot.clear { filter_by(:count).each { |_k, v| node_widget v } }
            end
            button "Method Time", margin: 4 do
               @units.text = "total time spent by the method alone in microseconds"
               @result_slot.clear { filter_by(:self_time).each { |_k, v| node_widget v } }
            end
            button "Total Time", margin: 4 do
               ...
               @result_slot.clear { filter_by(:total_time).each { |_k, v| node_widget v } }
            end
   ...
   def textscreen # get here from a visit(url)
      stack do
        ...
        # end
         ...
         puts "Script Run (Wall_time, sec) #{($shoes_profiler.prof_wall_end - $shoes_profiler.prof_wall_st) / 1_000_000.0}"
