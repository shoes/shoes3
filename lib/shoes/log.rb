module Shoes::LogWindow
      ...
      @main_slot = stack do
         flow do
            ...
            button "Copy", margin: 6, width: 80, height: 40 do
               self.clipboard = Shoes.log.collect { |typ, msg, at, _mid, rbf, rbl|
            ...
            button "Save", margin: 6, width: 80, height: 40 do
               ...
               if filename
                  File.open(filename, "w") do |f|
                     f.write(Shoes.log.collect do |typ, msg, at, _mid, rbf, rbl|
                        "#{typ.to_s.capitalize} in #{rbf} line #{rbl} | #{at}\n" +
                        "#{msg.to_s.force_encoding "UTF-8"}\n"
                     end.join("\n"))
                  end
   ...
   def update
      if @hash != Shoes.log.hash
         ...
         @log.clear do
            i = 0
            Shoes.log.each do |typ, msg, at, _mid, rbf, rbl|
               stack do
                  background "#f1f5e1" if i.even?
