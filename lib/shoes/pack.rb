   ...
   module Pack
      def self.rewrite(a, before, hsh)
         File.open(before) do |b|
            b.each do |line|
               a << line.gsub(/\#\{(\w+)\}/) { hsh[Regexp.last_match(1)] }
            end
         end
      end
      ...
      def self.exe(script, opt, &blk)
         ...
         if f2
            ...
            exe.save(script.gsub(/\.\w+$/, "") + ".exe") do |len|
               ...
               blk[last = prg] if blk && prg - last > 0.02 && prg < 1.0
            end
      ...
      def self.dmg(script, opt, &blk)
         ...
         vol_name = name.capitalize.gsub(/[-_](\w)/) { " " + Regexp.last_match(1).capitalize }
...
END
         ...
         dmg.chmod_file(0o755, "#{app_app}/Contents/MacOS/#{name}-launch")
         dmg.chmod_file(0o755, "#{app_app}/Contents/MacOS/cocoa-install")
      ...
      def self.linux(script, opt, &blk)
         ...
         if pkgf
            size = Shy.hrun(pkgf)
            if blk
               pblk = Shy.progress(size) do |_name, perc, _left|
                  blk[perc * 0.5]
               end
            end
         ...
         File.open(tgz_path, "wb") do |f|
            if blk
               pblk = Shy.progress(raw) do |_name, perc, _left|
                  blk[prog + (perc * prog)]
               end
            end
     ...
     systems. The defaults work."
         ...
         stack margin: 20 do
            ...
            def est_recount
               base =
                 case  @downOpt
                 when Shoes::I_NET then 98
                 when Shoes::I_YES then 11_600
                 when Shoes::I_NOV then 7000
               end
               base += (begin
                           (File.directory?(@path) ? Shy.du(@path) : File.size(@path))
                        rescue
                           0
                        end) / 1024
            ...
            def build_thread
               ...
               inc_win_text = @incWin.text
               inc_osx_text = @incOSX.text
               inc_linux_text = @incLinux.text
               Thread.start do
                  begin
                     sofar = 0.0
                     stage = 1.0 / [@shy.style[:checked], @exe.style[:checked], @dmg.style[:checked], @run.style[:checked]]
                       .select { |x| x }.size
                     blk = proc do |frac|
                     ...
                     if @shy_path
                        ...
                        pblk = Shy.progress(Shy.du(@path)) do |_name, perc, _left|
                           blk[perc]
                        end
